// Vokabeltrainer (PWA) – Central Sync Build FIX
...
function mergeCentralIntoLocal(central, enforced=false){
  ...
  for (const unit of Object.keys(central.units||{})){
    const list = central.units[unit]||[];
...
function initStats(){
  ...
  UNIT_META.forEach(u=>{ if (!stats.blocks[u.id]) stats.blocks[u.id]={correct:0,wrong:0}; if(!stats.words[u.id]) stats.words[u.id]={}; (vocab[u.id]||[]).forEach(w=>{ const k=key(w); if(!stats.words[u.id][k]) stats.words[u.id][k]={correct:0,wrong:0}; }); });
  saveStats(stats);
}
...
function record(originBlockId, item, ok){
  ...
  if (ok) { bs.correct++; ws.correct++; } else { bs.wrong++; ws.wrong++; } saveStats(stats); updateStatsUI();
}
...
function renderChecklist(){
  ...
  UNIT_META.forEach(u=>{ const label=document.createElement('label'); const cb=document.createElement('input'); cb.type='checkbox'; cb.value=u.id; cb.checked=false; cb.addEventListener('change', syncActiveBlockIds); label.appendChild(cb); label.appendChild(document.createTextNode(' '+u.name)); els.blockChecklist.appendChild(label); });
...
function updateInstallInfo(){ const standalone=window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone; els.installInfo.textContent=standalone?'installiert':'nicht installiert'; els.installBtn.style.display = standalone?'none':'inline-block'; }
...
els.nextBtn.addEventListener('click', ()=>{ const q=pickQuestion(); if(q) renderQuestion(); else { els.promptText.textContent='Bitte wähle mindestens einen Block.'; } });
...
if (els.forceCentralToggle) els.forceCentralToggle.addEventListener('change', ()=>{ const m=loadSync(); m.enforced = !!els.forceCentralToggle.checked; saveSync(m); });
if (els.refreshCentralBtn) els.refreshCentralBtn.addEventListener('click', async ()=>{ const central=await fetchCentral(); if(central){ const meta=loadSync(); mergeCentralIntoLocal(central, !!meta.enforced); saveSync({version: central.version||'unknown', enforced: !!meta.enforced}); if (els.centralVersion) els.centralVersion.textContent = central.version||'unknown'; alert('Zentrale Liste aktualisiert.'); } else alert('Zentrale Liste konnte nicht geladen werden.'); });
...
(async function init(){
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('./sw.js', { scope: './' }).then(reg=>{ if(reg.waiting) showUpdateBanner(); reg.addEventListener('updatefound',()=>{ const n=reg.installing; if(!n) return; n.addEventListener('statechange',()=>{ if(n.state==='installed' && navigator.serviceWorker.controller) showUpdateBanner(); }); }); });
